...
def cosine_similarity(x, y):
    ...
    norm_x = float(la.norm(x_arr))
    norm_y = float(la.norm(y_arr))
    denom = norm_x * norm_y

    # 3. 计算 cosine similarity，处理零向量情况
    if denom == 0.0:
        cos_sim = 0.0
    else:
        cos_sim = dot / denom  # 范围在 [-1, 1]

    # 4. 指数化，让相似度为正数
    exp_sim = math.exp(cos_sim)
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    # 2 & 3. 对每个单词获取 GloVe 向量并累加
    for word in words:
        # get_glove_embeddings 会内部处理 lower() 和 OOV 词
        word_vec = get_glove_embeddings(word, word_index_dict, embeddings, model_type)
        embedding += word_vec.astype(np.float32)
...
#     sentence = st.session_state.get("text_search", "")
...
#         # 1. 计算输入句子的 GloVe 平均向量
#         query_emb = averaged_glove_embeddings_gdrive(
#             sentence, word_index_dict, embeddings, model_type=model_type
#         )

#         # 2. 为每个类别计算并缓存 GloVe 向量
...
#                     category, word_index_dict, embeddings, model_type=model_type
#                 )
#             cat_emb = st.session_state[cache_key][category]
#             # 3. 计算余弦相似度（已在函数内做指数化）
#             cosine_sim[idx] = cosine_similarity(query_emb, cat_emb)
...
#         # 1. 确保类别 embedding 已经缓存
#         get_category_embeddings(embeddings_metadata)

#         # 计算输入句子的 OpenAI 向量
#         query_emb = get_openai_embeddings(sentence, model_name=model_name)
...
#         ##########################################
#         # ✅ 每次都用当前 categories 重新更新类别 embedding
#         get_category_embeddings(embeddings_metadata)

#         # 计算输入句子的 sentence-transformer 向量
#         if model_name:
#             query_emb = get_sentence_transformer_embeddings(sentence, model_name=model_name)
#         else:
#             query_emb = get_sentence_transformer_embeddings(sentence)

#     # 对每个类别计算相似度
#     for idx, category in enumerate(categories):
#         cat_emb = st.session_state[cache_key][category]
#         cosine_sim[idx] = cosine_similarity(query_emb, cat_emb)
#         # 2. 计算输入句子的 sentence-transformer 向量
#         if model_name:
#             query_emb = get_sentence_transformer_embeddings(sentence, model_name=model_name)
#         else:
#             query_emb = get_sentence_transformer_embeddings(sentence)
...
#     # 将 {类别索引: 分数} 转成排好序的列表
#     sorted_cosine_scores_items = sorted(
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    NOTE: This function handles three embedding types:
    - GloVe (word averaging)
    - OpenAI embeddings
    ...
    # 当前输入句子（UI 里 key="text_search"）
    sentence = st.session_state.get("text_search", "")

    embedding_model = embeddings_metadata["embedding_model"]

    # --------------------------------------------------
    # 1) GloVe: averaged word embeddings
    # --------------------------------------------------
    ...
    # --------------------------------------------------
        ...
        # 同样，每次根据当前 categories 更新类别向量
        get_category_embeddings(embeddings_metadata)

        # 1. 计算输入句子的 sentence-transformer 向量
        if model_name:
            query_emb = get_sentence_transformer_embeddings(
        ...
        st.error(f"Unknown embedding_model: {embedding_model}")
        return []
...
if __name__ == "__main__":
    ...
    if st.session_state.text_search:
        ...
        with st.spinner("Obtaining Cosine similarity for Glove..."):
            ...
            latency_dict["glove_" + str(model_type)] = t1 - t0
        ...
        with st.spinner("Obtaining Cosine similarity for 384d sentence transformer..."):
            ...
            latency_dict["sentence_transformer_384"] = t1 - t0
        ...
        with st.spinner("Obtaining Cosine similarity for OpenAI Small (1536d)..."):
            ...
            latency_dict["openai_small_1536"] = t1 - t0 
        ...
        with st.spinner("Obtaining Cosine similarity for OpenAI Large (3072d)..."):
            ...
            latency_dict["openai_large_3072"] = t1 - t0
        ...
        for model_name, scores in results_dict.items():
            ...
            latency = latency_dict.get(model_name, None)
            comparison_data.append({
                "Model": model_name,
                "Top Category": top_category,
                "Confidence Score": f"{top_score:.4f}",
                "Latency (s)": f"{latency:.4f}" if latency is not None else "N/A",
            })
